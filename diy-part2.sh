...
# 复制自定义的 index.htm（添加存在性检查，避免编译失败）
CUSTOM_INDEX="$GITHUB_WORKSPACE/files/usr/lib/lua/luci/view/admin_status/index.htm"
if [ -f "$CUSTOM_INDEX" ]; then
    cp -f "$CUSTOM_INDEX" feeds/luci/modules/luci-mod-status/luasrc/view/admin_status/index.htm
    echo "✅ 已应用自定义 index.htm（CPU 温度显示）"
else
    echo "⚠️ 未找到自定义 index.htm，跳过 CPU 温度显示功能"
fi
