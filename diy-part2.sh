...
# 在新版 APK 模式下，直接改源码文件易失效，使用 uci-defaults 脚本确保开机即生效
...
# 3. 补齐 Argon 可视化配置插件 (绕过 diy1 中的索引错误)
...
# 5. 剔除真正导致编译冲突的 onionshare
rm -rf feeds/packages/net/onionshare-cli
echo '# CONFIG_PACKAGE_onionshare-cli is not set' >> .config

# 6. 注入 J1900 硬件驱动和补齐 PassWall 必需的 PPP 模块
cat << 'EOF' >> .config
...
# 7. 自动更新 PassWall 核心组件 (根据 GitHub 最新 Release)
update_go_package() {
    local pkg_name=$1
    local github_repo=$2
